...
tf.app.flags.DEFINE_float('learning_rate', 0.00035, 'Initial learning rate.')
...
tf.app.flags.DEFINE_string('checkpoint_exclude_scopes', ['logits', 'concat_comb', 'fusion', 'Distributions'], 'Comma-separated list of scopes of variables to exclude when restoring from a checkpoint.')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        for loss in tf.get_collection(tf.GraphKeys.LOSSES, first_clone_scope):
            ...
            if 'ver' in loss.name:
                loss_dict['ver'] = loss
            elif 'distribution' in loss.name:
                loss_dict['trip_distribution'] = loss
            elif 'instance' in loss.name:
                loss_dict['entropy_across_instance'] = loss
